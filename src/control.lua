...
local control = {
	...
	on_gui_click = function(event)
		...
		elseif event.element.name == "settingsbutton_2" 	then this.settingsbutton_2_onClick(player)
	...
	on_tick = function(event)
		...
		end, function(e) end, function(e) end)
		...
		if day_time_m < 10 then
			...
			day_time_m_str = tostring(day_time_m)
		end
		if day_time_h < 10 then
			day_time_h_str= "0" .. day_time_h
		else
			day_time_h_str= tostring(day_time_h)
		end
